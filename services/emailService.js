    ...
    <body style="margin:0;padding:20px;background:#111;">
      <div style="${BASE_STYLE}">
        <div style="background:linear-gradient(135deg,#1a1a1a,#0d0d0d);padding:30px;text-align:center;border-bottom:2px solid ${GOLD};">
          <h1 style="color:${GOLD};margin:0;font-size:28px;letter-spacing:3px;">VYTAL</h1>
        ...
        <div style="background:#111;padding:20px;text-align:center;border-top:1px solid #222;">
          <p style="color:#555;font-size:11px;margin:0;">
            © ${new Date().getFullYear()} Vytal. Todos los derechos reservados.<br>
...
async function sendEmail(to, subject, htmlContent) {
  try {
    ...
    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM || `Vytal <${process.env.SMTP_USER}>`,
      to,
      subject,
...
async function sendNewUserNotificationEmail(user) {
  ...
  return sendEmail(
    adminEmail,
    `🆕 Nuevo usuario en Vytal: ${user.name}`,
    `
    <h2 style="color:${GOLD};margin-top:0;">Nuevo registro en Vytal</h2>
    ...
    <div style="text-align:center;margin-top:25px;">
      <a href="${process.env.APP_URL}/dashboard.html" style="background:${GOLD};color:#0d0d0d;padding:12px 28px;text-decoration:none;border-radius:6px;font-weight:bold;font-size:14px;display:inline-block;">Ir al panel de Vytal</a>
    </div>
    `
  );
}
...
async function sendWelcomeEmail(user, nextBillingDate) {
  ...
  return sendEmail(
    user.email,
    '¡Bienvenido/a a Vytal Pro! Tu plan está activo 🏆',
    `
    <h2 style="color:${GOLD};margin-top:0;">¡Hola, ${user.name}!</h2>
    <p style="line-height:1.7;color:#ccc;">Nos alegra tenerte en la familia <strong style="color:${GOLD};">Vytal</strong>. Tu plan Pro está activo y tu plan personalizado de nutrición y entrenamiento ya está disponible.</p>
...
async function sendPaymentConfirmedEmail(user, amount, nextBillingDate) {
  ...
  return sendEmail(
    user.email,
    `Pago confirmado: ${amount} € — Vytal`,
    `
    <h2 style="color:${GOLD};margin-top:0;">Pago recibido ✓</h2>
    <p style="color:#ccc;">Hemos procesado correctamente tu pago de <strong style="color:#fff;">${amount} €</strong>. Gracias por continuar con Vytal.</p>
...
async function sendCancellationEmail(user, effectiveDate) {
  ...
  return sendEmail(
    user.email,
    'Has vuelto al plan gratuito — Vytal',
    `
    <h2 style="color:#ccc;margin-top:0;">Has vuelto al plan gratuito</h2>
    <p style="color:#aaa;line-height:1.7;">Hemos dejado de cobrarte el plan Pro. Seguirás teniendo acceso a tu plan hasta el <strong style="color:#fff;">${fecha}</strong> y después seguirás usando Vytal gratis.</p>
    <p style="color:#777;font-size:13px;">Si cambiaste de opinión, puedes actualizar a Pro en cualquier momento desde tu panel.</p>
    <div style="text-align:center;margin-top:25px;">
      <a href="${process.env.APP_URL}" style="background:#1a1a1a;color:${GOLD};padding:12px 28px;text-decoration:none;border-radius:6px;font-weight:bold;border:1px solid ${GOLD};display:inline-block;">Volver a Vytal</a>
    </div>
    `
  );
}
...
async function sendNutritionPlanEmail(user, plan) {
  return sendEmail(
    user.email,
    `Tu Plan Personalizado Vytal está listo 🥗`,
    `
